...
8. [Boas Práticas e Workflows](#8-boas-práticas-e-workflows)
...
- Simplificar eixos
- Evitar 3D gratuitoI'll continue creating the enhanced documentation with more theory.
...
**Definição:** Métodos para resumir e descrever características principais de um conjunto de dados.

**Objetivos:**
1. Resumir grandes volumes de dados
2. Identificar padrões centrais
3. Medir variabilidade
4. Detectar anomalias
5. Preparar para análise inferencial
...
- Variável contínua
- Quando todos os dados são importantes

**Propriedades:**
- Sensível a outliers
- Única solução
- Útil para cálculos posteriores
- Pode não existir nos dados

**Interpretação:**
- Valor "típico" se distribuição simétrica
- Centro de gravidade dos dados

```python
dados = [2, 4, 4, 4, 5, 5, 7, 9]

media = np.mean(dados)  # 5.0

# Com outlier
dados_com_outlier = dados + [100]
media_outlier = np.mean(dados_com_outlier)  # 15.56 (distorcida)
```

**2. Mediana**

**Definição:** Valor central quando dados estão ordenados.
...
- Dados ordinais
- Renda, preços (geralmente)

**Propriedades:**
- Robusta a outliers
- Sempre existe nos dados (n ímpar)
- Divide dados em 50/50
- Pode não ser única (empates)

**Interpretação:**
- 50% dos dados estão abaixo
- 50% dos dados estão acima

```python
mediana = np.median(dados)  # 4.5

# Com outlier - não afeta muito
mediana_outlier = np.median(dados_com_outlier)  # 5.0 (robusta)
```
...
moda = moda_resultado[0][0]  # 4

# Dados categóricos
categorias = ['A', 'B', 'B', 'C', 'B', 'A']
moda_cat = max(set(categorias), key=categorias.count)  # 'B'
```
...
|---|---|---|---|
| Sensibilidade a outliers | Alta | Baixa | Nenhuma |
| Tipo de dados | Numérico | Numérico/Ordinal | Qualquer |
| Sempre existe | Sim | Sim | Não |
| Unicidade | Sim | Quase sempre | Não |
| Uso em cálculos | Sim | Limitado | Não |

#### Medidas de Dispersão

**1. Variância**

**Definição:** Média dos quadrados dos desvios da média.

**Fórmula:**
- População: σ² = Σ(x - μ)² / N
- Amostra: s² = Σ(x - x̄)² / (n-1)
...
**Interpretação:**
- Alta: Dados espalhados
- Baixa: Dados concentrados
- Zero: Todos valores iguais

```python
variancia = np.var(dados)  # Populacional (padrão)
variancia_amostra = np.var(dados, ddof=1)  # Amostral
```
...
- Interpretar dispersão
- Regra 68-95-99.7 (normal)

**Vantagens sobre variância:**
- Mesma unidade dos dados
- Mais interpretável
- Comparável com média
...
# Aplicar regra empírica
media = np.mean(dados)
limite_inferior = media - 2 * desvio_padrao
limite_superior = media + 2 * desvio_padrao
# 95% dos dados deveriam estar neste intervalo
```

**3. Amplitude**

**Definição:** Diferença entre máximo e mínimo.

**Quando usar:**
- Visão rápida do range
- Dados sem outliers
- Complementar outras medidas

**Limitações:**
- Muito sensível a outliers
- Ignora distribuição interna
...
**Definição:** Diferença entre terceiro e primeiro quartis.

**Fórmula:** IQR = Q3 - Q1

**Quando usar:**
- Dados com outliers
- Alternativa robusta ao desvio padrão
- Identificar outliers
...
**Definição:** Medida de assimetria da distribuição.
...
- Negativa: Moda > Mediana > Média
- Afeta escolha de testes estatísticos
...
if abs(assimetria) < 0.5:
    ...
    print("Assimetria positiva (cauda direita)")
elif assimetria < -0.5:
    print("Assimetria negativa (cauda esquerda)")
```

**2. Curtose (Kurtosis)**

**Definição:** Medida de "achatamento" da distribuição.

**Valores (excess kurtosis):**
- Kurtosis = 0: Mesocúrtica (como normal)
- Kurtosis > 0: Leptocúrtica (pico alto, caudas pesadas)
- Kurtosis < 0: Platicúrtica (achatada, caudas leves)

**Interpretação:**
- Alta: Mais outliers
- Baixa: Menos outliers
- Importante para risco e volatilidade

```python
curtose = stats.kurtosis(dados)  # Excess kurtosis

if curtose > 0:
    print("Leptocúrtica: caudas pesadas, mais outliers")
elif curtose < 0:
    print("Platicúrtica: caudas leves, menos outliers")
else:
    print("Mesocúrtica: similar à normal")
```

### 6.2 Correlação e Covariância

**Covariância:**

**Definição:** Medida de como duas variáveis variam juntas.

**Fórmula:** Cov(X,Y) = Σ(xᵢ - x̄)(yᵢ - ȳ) / (n-1)

**Interpretação:**
- Cov > 0: Variam na mesma direção
- Cov < 0: Variam em direções opostas
- Cov = 0: Sem relação linear

**Limitações:**
- Depende de escala das variáveis
- Difícil de interpretar magnitude
- Não indica força da relação
...
**Definição:** Medida padronizada de relação linear entre variáveis.
...
**Definição:** Correlação de Pearson aplicada aos ranks.

**Quando usar:**
- Relação monotônica (não necessariamente linear)
- Dados ordinais
- Presença de outliers
...
```python
correlacao_spearman, p_value = stats.spearmanr(x, y)
```

**Correlação vs Causação:**

**IMPORTANTE:** Correlação não implica causação!

**Possibilidades:**
1. X causa Y
2. Y causa X
3. Z causa X e Y (variável confundidora)
4. Coincidência

**Para estabelecer causação:**
- Experimento controlado
- Temporalidade (causa antes do efeito)
- Eliminar variáveis confundidoras
- Mecanismo plausível

### 6.3 Testes de Hipóteses

**Definição:** Procedimento estatístico para tomar decisões baseado em evidências de dados.
...
**Exemplos:**
- H₀: μ = 100 (média é 100)
- H₁: μ ≠ 100 (média é diferente de 100)

**2. Nível de Significância (α):**

**Definição:** Probabilidade de rejeitar H₀ quando é verdadeira (Erro Tipo I).

**Valores comuns:**
- α = 0.05 (5%): Padrão
- α = 0.01 (1%): Mais rigoroso
- α = 0.10 (10%): Mais liberal

**3. P-valor:**

**Definição:** Probabilidade de observar resultado tão extremo quanto o obtido, assumindo H₀ verdadeira.

**Interpretação:**
- p < α: Rejeitar H₀ (evidência significativa)
- p ≥ α: Não rejeitar H₀ (evidência insuficiente)

**Importante:**
- P-valor NÃO é probabilidade de H₀ ser verdadeira
- P-valor pequeno ≠ efeito grande
...
|---|---|---|
| **Não Rejeitar H₀** | Correto (1-α) | Erro Tipo II (β) |
...
**Objetivo:** Testar se média de uma amostra difere de valor específico.

```python
# H₀: μ = 100
# H₁: μ ≠ 100

dados = [95, 102, 98, 101, 97, 103, 99]
valor_teste = 100

t_stat, p_value = stats.ttest_1samp(dados, valor_teste)

print(f"Estatística t: {t_stat:.4f}")
print(f"P-valor: {p_value:.4f}")

if p_value < 0.05:
    print("Rejeitamos H₀: média é diferente de 100")
else:
    print("Não rejeitamos H₀: evidência insuficiente")
```

**2. Teste t de duas amostras independentes:**

**Objetivo:** Comparar médias de dois grupos independentes.

**Pressupostos:**
- Independência entre grupos
- Distribuição aproximadamente normal
- Variâncias aproximadamente iguais (teste de Levene)

```python
# H₀: μ₁ = μ₂
# H₁: μ₁ ≠ μ₂

grupo1 = [23, 25, 27, 22, 24, 26]
grupo2 = [30, 32, 28, 31, 33, 29]
...
if p_value < 0.05:
    print("Grupos têm médias diferentes")
else:
...
**Objetivo:** Comparar médias de medidas repetidas (antes/depois).

**Quando usar:**
- Mesmos indivíduos em dois momentos
- Pares matched
- Medidas repetidas

```python
# Antes e depois de tratamento
antes = [120, 135, 128, 142, 138]
depois = [115, 128, 122, 135, 130]

t_stat, p_value = stats.ttest_rel(antes, depois)

if p_value < 0.05:
    print("Houve mudança significativa")
```
...
- Testar goodness of fit

**1. Teste de Independência:**

**Objetivo:** Testar se duas variáveis categóricas são independentes.
...
**Definição:** Analysis of Variance - comparar médias de múltiplos grupos.
...
**Hipóteses:**
- H₀: μ₁ = μ₂ = ... = μₖ (todas médias iguais)
- H₁: Pelo menos uma média diferente

```python
grupo1 = [23, 25, 27, 22, 24]
grupo2 = [30, 32, 28, 31, 33]
grupo3 = [20, 22, 21, 19, 23]
...
if p_value < 0.05:
    print("Pelo menos um grupo é diferente")
    print("Realizar testes post-hoc para identificar quais")
```

**Testes Post-hoc:**

Após ANOVA significativa, identificar quais grupos diferem:

```python
from statsmodels.stats.multicomp import pairwise_tukeyhsd

# Preparar dados
dados_todos = grupo1 + grupo2 + grupo3
grupos = (['G1']*len(grupo1) + ['G2']*len(grupo2) +
          ['G3']*len(grupo3))

# Tukey HSD
tukey = pairwise_tukeyhsd(dados_todos, grupos, alpha=0.05)
print(tukey)
```
...
- Muitos testes paramétricos assumem normalidade
...
**Quando usar:**
- Amostras maiores
- Comparar com qualquer distribuição

```python
stat, p_value = stats.kstest(dados, 'norm')
```
...
# Se pontos próximos à linha: aproximadamente normal
```
...
**Definição:** Campo da inteligência artificial que permite computadores aprenderem a partir de dados sem programação explícita.

**Tipos de Aprendizado:**

**1. Supervisionado:**

**Definição:** Aprendizado com dados rotulados (input + output conhecido).

**Objetivo:** Aprender função f: X → Y

**Quando usar:**
- Temos exemplos de entrada e saída
- Queremos predizer novos casos
...
**Objetivo:** Predizer categoria/classe.

**Tipos:**
- **Binária:** 2 classes (sim/não, spam/ham)
- **Multiclasse:** > 2 classes (tipo de produto)
- **Multilabel:** Múltiplas classes por instância
...
- Temperatura futura
- Vendas próximo mês
- Salário baseado em experiência
...
**Definição:** Aprendizado sem rótulos (apenas input).
...
- Encontrar grupos naturais

**Técnicas:**

**a) Clustering:**

**Objetivo:** Agrupar observações similares.
...
**Definição:** Aprendizado através de interação e recompensas.
...
### 7.2 Pipeline de Machine Learning

**Fluxo completo:**

```
Definição Problema → Coleta Dados → Limpeza →
Feature Engineering → Divisão Train/Test →
Escolha Modelo → Treinamento → Validação →
Ajuste Hiperparâmetros → Avaliação Final → Deploy
```
...
**Definição:** Processo de criar novas features ou transformar existentes para melhorar modelo.

**Técnicas:**

**a) Criação de Features:**
...
df['faixa_idade'] = pd.cut(df['idade'],
                            bins=[0, 18, 35, 60, 100],
                            labels=['Jovem', 'Adulto', 'Meia-idade', 'Senior'])
```
...
**Objetivo:** Escalar features para range [0, 1].

**Fórmula:** x_norm = (x - min) / (max - min)

**Quando usar:**
- Algoritmos sensíveis a escala (KNN, SVM, Redes Neurais)
- Quando range conhecido é importante
...
**Objetivo:** Média 0, desvio padrão 1.

**Fórmula:** x_std = (x - μ) / σ

**Quando usar:**
- Features com distribuições diferentes
- Algoritmos assumem dados centrados (Regressão, PCA)
- Presença de outliers (mais robusto que min-max)
...
df['valor_log'] = np.log1p(df['valor'])  # log(1+x)
...
transformed, lambda_param = boxcox(df['valor'])
```

**c) Encoding de Variáveis Categóricas:**

**Label Encoding:**

**Definição:** Converter categorias em números.

**Quando usar:**
- Variável target (classificação)
- Variáveis ordinais (Baixo=1, Médio=2, Alto=3)
- Algoritmos baseados em árvores

**Cuidado:** Não usar para categorias nominais em regressão linear (cria ordem artificial).
...
**Definição:** Criar coluna binária para cada categoria.
...
- Usar drop='first' para evitar dummy trap

```python
# Pandas
df_encoded = pd.get_dummies(df, columns=['cidade'], drop_first=True)

# Scikit-learn
from sklearn.preprocessing import OneHotEncoder

encoder = OneHotEncoder(drop='first', sparse=False)
encoded = encoder.fit_transform(df[['cidade']])
```

**Target Encoding:**

**Definição:** Substituir categoria pela média do target naquela categoria.

**Quando usar:**
- Alta cardinalidade (muitas categorias)
- Problemas de classificação
- Com validação cruzada para evitar overfitting
...
**Propósito:** Avaliar generalização do modelo.
...
Manter mesma proporção de classes em treino e teste.
...
# Com estratificação (classificação)
X_train, X_test, y_train, y_test = train_test_split(
...
# Resultado: 70% treino, 15% validação, 15% teste
```

Este arquivo está muito extenso. Vou criar uma versão mais concisa mas ainda muito rica em teoria. Devo continuar ou criar um novo arquivo otimizado?